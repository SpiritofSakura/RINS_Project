...
class RingLocalizator(Node):
    def __init__(self):
        ...
        # threshold for number of detections to consider a location as a ring location
        ...
        # Subscribe to ring markers from detect_rings
        ...
        # Publisher for persistent detected ring locations
        ...
        self.get_logger().info("Ring localizator node initialized. Subscribing to /ring_marker")
    
    def marker_callback(self, marker_msg):
        """
        Callback for incoming ring markers from detect_rings.
        ...
        try:
            ...
            self.get_logger().debug(
                f"Ring detected at map coords: ({x_map:.2f}, {y_map:.2f})"
            )
    ...
    def check_and_mark_location(self, x, y):
        """
        Check if there are 15+ detections within 1m radius (2D) of (x, y).
        ...
        # If 15+ detections and location not already marked, mark it
        if detections_in_radius >= self.threshold_detections:
            ...
            if not already_marked:
                ...
                self.get_logger().info(
                    f"Ring location marked! Total detections: {detections_in_radius} "
                    f"at ({x:.2f}, {y:.2f})"
                )
    ...
    def publish_persistent_marker(self, x, y):
        ...
        # Set color (blue for detected rings)
        marker.color.r = 0.0
        marker.color.g = 0.0
        marker.color.b = 1.0
        marker.color.a = 1.0
...
def main():
    print('Ring localizator node starting.')
